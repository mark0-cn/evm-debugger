...
pub async fn start_server(bind_addr: &str) -> anyhow::Result<ServerHandle> {
...
pub async fn run_server(bind_addr: &str) -> anyhow::Result<()> {
    let handle = start_server(bind_addr).await?;
    tracing::info!("EVM Debugger listening on http://{}", handle.addr);
    handle.wait().await
}
